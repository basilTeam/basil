...
namespace basil {
    ...
    vector<u32> allocatable_registers() {
        switch (_arch) {
            case X86_64: {
                using namespace x64;
                static Register ALLOCATABLE_REGISTERS[] = {RBX, R8, R9, R10, R11, R12, R13, R14, R15};
                vector<u32> regs;
    ...
    x64::Arg x64_arg(const Location& src) {
        using namespace x64;
        switch (src.type) {
            case LOC_NONE: return imm64(0);
            case LOC_REGISTER: return r64((Register)src.reg);
            case LOC_LABEL: return label64(global((const char*)label_of(src).raw()));
            case LOC_LOCAL:
                if (local_of(src).reg >= 0) return r64((Register)local_of(src).reg);
                else
                    return m64(RBP, local_of(src).offset);
            case LOC_CONSTANT: return label64(global((const char*)constant_of(src).name.raw()));
            case LOC_IMMEDIATE: return imm64(src.immediate);
        }
    }
    ...
    void x64_binary(const x64::Arg& dest, const x64::Arg& lhs, const x64::Arg& rhs, const x64::Arg& clobber,
                    void (*op)(const x64::Arg&, const x64::Arg&, x64::Size)) {
        using namespace x64;
    ...
    void load(const Location& dest, const Location& src, u32 offset) {
        switch (_arch) {
            case X86_64: {
                using namespace x64;
                auto d = x64_arg(dest), s = x64_to_register(x64_arg(src), x64_clobber(1));
            ...
            case X86_64: {
                ...
                if (is_register(d.type) && !is_memory(l.type) && !is_memory(r.type)) {
                    if (d.data.reg == l.data.reg) // no need for two steps
                        if (is_immediate(r.type) && r.data.imm64 == 1) return inc(d);
                        else if (is_immediate(r.type) && r.data.imm64 == -1)
                            return dec(d);
                        else
                            return add(d, r);
                    else if (d.data.reg == r.data.reg) // no need for two steps
                        if (is_immediate(l.type) && l.data.imm64 == 1) return inc(d);
    ...
    void sub(const Location& dest, const Location& lhs, const Location& rhs) {
        switch (_arch) {
            case X86_64: {
                ...
                if (is_register(d.type) && !is_memory(l.type) && !is_memory(r.type)) {
                    if (d.data.reg == l.data.reg) // no need for two steps
                        if (is_immediate(r.type) && r.data.imm64 == 1) return dec(d);
                        else if (is_immediate(r.type) && r.data.imm64 == -1)
                            return inc(d);
                        else
                            return sub(d, r);
    ...
    void and_op(const Location& dest, const Location& lhs, const Location& rhs) {
        switch (_arch) {
            case X86_64: x64_binary(x64_arg(dest), x64_arg(lhs), x64_arg(rhs), x64_clobber(0), x64::and_); return;
            default: return;
        }
    }
    ...
    void or_op(const Location& dest, const Location& lhs, const Location& rhs) {
        switch (_arch) {
            case X86_64: x64_binary(x64_arg(dest), x64_arg(lhs), x64_arg(rhs), x64_clobber(0), x64::or_); return;
            default: return;
        }
    }
    ...
    void xor_op(const Location& dest, const Location& lhs, const Location& rhs) {
        switch (_arch) {
            case X86_64: x64_binary(x64_arg(dest), x64_arg(lhs), x64_arg(rhs), x64_clobber(0), x64::xor_); return;
            default: return;
        }
    }
    ...
    void jump(const Location& dest) {
        switch (_arch) {
            case X86_64: x64::jmp(x64_arg(dest)); return;
            default: return;
        }
    }
    ...
    void set_arg(u32 i, const Location& src) {
        switch (_arch) {
            case X86_64: x64_move(x64_param(i), x64_arg(src), x64_clobber(0)); return;
            default: return;
        }
    }
    ...
    void get_arg(const Location& dest, u32 i) {
        switch (_arch) {
            case X86_64: x64_move(x64_arg(dest), x64_param(i), x64_clobber(0)); return;
            default: return;
        }
    }
    ...
    void call(const Location& func) {
        switch (_arch) {
            case X86_64: x64::call(x64_arg(func)); return;
            default: return;
        }
    }
    ...
    void global_label(const string& name) {
        switch (_arch) {
            case X86_64: x64::label(jasmine::global((const char*)name.raw())); return;
            default: return;
        }
    }
    ...
    void local_label(const string& name) {
        switch (_arch) {
            case X86_64: x64::label(jasmine::local((const char*)name.raw())); return;
            default: return;
        }
    }
    ...
    void push(const Location& src) {
        switch (_arch) {
            case X86_64: x64::push(x64_arg(src)); return;
            default: return;
        }
    }
    ...
    void pop(const Location& dest) {
        switch (_arch) {
            case X86_64: x64::pop(x64_arg(dest)); return;
            default: return;
        }
    }
