...
Value intro_set_section(ref<Env> env, const Value& args) {
    if (!args.get_product()[0].is_int()) {
        err(args.get_product()[0].loc(), "Expected integer for section number, given ", args.get_product()[0]);
        return Value(ERROR);
    }
    if (args.get_product()[0].get_int() < 1 || args.get_product()[0].get_int() > 7) {
        err(args.get_product()[0].loc(), "Invalid section number. Must be between 1 ", "and 7.");
        return Value(ERROR);
    }
