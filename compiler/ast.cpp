...
namespace basil {
    ASTNode::ASTNode(SourceLocation loc) : _loc(loc), _type(nullptr) {}
    ...
    ASTSingleton::ASTSingleton(const Type* type) : ASTNode(NO_LOCATION), _type(type) {}
    ...
    ASTVoid::ASTVoid(SourceLocation loc) : ASTNode(loc) {}
    ...
    ASTInt::ASTInt(SourceLocation loc, i64 value) : ASTNode(loc), _value(value) {}
    ...
    ASTSymbol::ASTSymbol(SourceLocation loc, u64 value) : ASTNode(loc), _value(value) {}
    ...
    ASTString::ASTString(SourceLocation loc, const string& value) : ASTNode(loc), _value(value) {}
    ...
    ASTBool::ASTBool(SourceLocation loc, bool value) : ASTNode(loc), _value(value) {}
    ...
    ASTVar::ASTVar(SourceLocation loc, const ref<Env> env, u64 name) : ASTNode(loc), _env(env), _name(name) {}

    const Type* ASTVar::lazy_type() {
        const Def* def = _env->find(symbol_for(_name));
        if (def && def->value.is_runtime()) return ((const RuntimeType*)def->value.type())->base();
    ...
    ASTExtern::ASTExtern(SourceLocation loc, const Type* type) : ASTNode(loc), _type(type) {}
    ...
    ASTUnary::ASTUnary(SourceLocation loc, ASTNode* child) : ASTNode(loc), _child(child) {
        _child->inc();
    }
    ...
    ASTBinaryMath::ASTBinaryMath(SourceLocation loc, ASTMathOp op, ASTNode* left, ASTNode* right)
    ...
    const Type* ASTBinaryMath::lazy_type() {
        ...
        if (result != INT) {
            err(loc(), "Invalid parameters to arithmetic expression: '", _left->type(), "' and '", _right->type(),
                "'.");
            return ERROR;
        }
        return result;
    ...
    ref<SSANode> ASTBinaryMath::emit(ref<BasicBlock>& parent) {
        switch (_op) {
            case AST_ADD: return newref<SSABinary>(parent, type(), SSA_ADD, _left->emit(parent), _right->emit(parent));
            case AST_SUB: return newref<SSABinary>(parent, type(), SSA_SUB, _left->emit(parent), _right->emit(parent));
            case AST_MUL: return newref<SSABinary>(parent, type(), SSA_MUL, _left->emit(parent), _right->emit(parent));
            case AST_DIV: return newref<SSABinary>(parent, type(), SSA_DIV, _left->emit(parent), _right->emit(parent));
            case AST_REM: return newref<SSABinary>(parent, type(), SSA_REM, _left->emit(parent), _right->emit(parent));
    ...
    Location ASTBinaryMath::emit(Function& func) {
        switch (_op) {
            case AST_ADD: return func.add(new AddInsn(_left->emit(func), _right->emit(func)));
            case AST_SUB: return func.add(new SubInsn(_left->emit(func), _right->emit(func)));
            case AST_MUL: return func.add(new MulInsn(_left->emit(func), _right->emit(func)));
            case AST_DIV: return func.add(new DivInsn(_left->emit(func), _right->emit(func)));
            case AST_REM: return func.add(new RemInsn(_left->emit(func), _right->emit(func)));
    ...
    ASTBinaryLogic::ASTBinaryLogic(SourceLocation loc, ASTLogicOp op, ASTNode* left, ASTNode* right)
        : ASTBinary(loc, left, right), _op(op) {}
    ...
    const Type* ASTBinaryLogic::lazy_type() {
        ...
        if (result != BOOL) {
            err(loc(), "Invalid parameters to logical expression: '", _left->type(), "' and '", _right->type(), "'.");
            return ERROR;
        }
        return BOOL;
    }
    ...
    ref<SSANode> ASTBinaryLogic::emit(ref<BasicBlock>& parent) {
        switch (_op) {
            case AST_AND: return newref<SSABinary>(parent, type(), SSA_AND, _left->emit(parent), _right->emit(parent));
            case AST_OR: return newref<SSABinary>(parent, type(), SSA_OR, _left->emit(parent), _right->emit(parent));
            case AST_XOR: return newref<SSABinary>(parent, type(), SSA_XOR, _left->emit(parent), _right->emit(parent));
            default: return nullptr;
        }
    }
    ...
    Location ASTBinaryLogic::emit(Function& func) {
        switch (_op) {
            case AST_AND: return func.add(new AndInsn(_left->emit(func), _right->emit(func)));
            case AST_OR: return func.add(new OrInsn(_left->emit(func), _right->emit(func)));
            case AST_XOR: return func.add(new XorInsn(_left->emit(func), _right->emit(func)));
            default: return loc_none();
        }
    }

    static const char* LOGIC_OP_NAMES[] = {"and", "or", "xor", "not"};

    void ASTBinaryLogic::format(stream& io) const {
        write(io, "(", LOGIC_OP_NAMES[_op], " ", _left, " ", _right, ")");
    }

    ASTNot::ASTNot(SourceLocation loc, ASTNode* child) : ASTUnary(loc, child) {}

    const Type* ASTNot::lazy_type() {
        if (_child->type() == ERROR) return ERROR;
        if (unify(_child->type(), BOOL) != BOOL) {
            err(loc(), "Invalid argument to 'not' expression: '", _child->type(), "'.");
            return ERROR;
        }
        return BOOL;
    }
    ...
    ASTBinaryEqual::ASTBinaryEqual(SourceLocation loc, ASTEqualOp op, ASTNode* left, ASTNode* right)
        : ASTBinary(loc, left, right), _op(op) {}
    ...
    ref<SSANode> ASTBinaryEqual::emit(ref<BasicBlock>& parent) {
        switch (_op) {
            case AST_EQUAL: return newref<SSABinary>(parent, type(), SSA_EQ, _left->emit(parent), _right->emit(parent));
            case AST_INEQUAL:
    ...
    Location ASTBinaryEqual::emit(Function& func) {
        ...
        switch (_op) {
            case AST_EQUAL: return func.add(new EqualInsn(_left->emit(func), _right->emit(func)));
            case AST_INEQUAL: return func.add(new InequalInsn(_left->emit(func), _right->emit(func)));
            default: return loc_none();
        }
    }
    ...
    ASTBinaryRel::ASTBinaryRel(SourceLocation loc, ASTRelOp op, ASTNode* left, ASTNode* right)
        : ASTBinary(loc, left, right), _op(op) {}
    ...
    const Type* ASTBinaryRel::lazy_type() {
        ...
        if (result != INT) {
            ...
            result = unify(lt, rt);
            if (result != STRING) {
                err(loc(), "Invalid parameters to relational expression: '", _left->type(), "' and '", _right->type(),
                    "'.");
                return ERROR;
            }
        }
    ...
    Location ASTBinaryRel::emit(Function& func) {
        if (_left->type() == STRING || _right->type() == STRING) {
            ...
            switch (_op) {
                case AST_LESS: return func.add(new LessInsn(result, loc_immediate(0)));
                case AST_LESS_EQUAL: return func.add(new LessEqualInsn(result, loc_immediate(0)));
                case AST_GREATER: return func.add(new GreaterInsn(result, loc_immediate(0)));
                case AST_GREATER_EQUAL: return func.add(new GreaterEqualInsn(result, loc_immediate(0)));
                default: return loc_none();
            }
        }

        switch (_op) {
            case AST_LESS: return func.add(new LessInsn(_left->emit(func), _right->emit(func)));
            case AST_LESS_EQUAL: return func.add(new LessEqualInsn(_left->emit(func), _right->emit(func)));
            case AST_GREATER: return func.add(new GreaterInsn(_left->emit(func), _right->emit(func)));
            case AST_GREATER_EQUAL: return func.add(new GreaterEqualInsn(_left->emit(func), _right->emit(func)));
            default: return loc_none();
        }
    }
    ...
    Location ASTCall::emit(Function& func) {
        ...
        for (u32 i = 0; i < _args.size(); i++) { arglocs.push(_args[i]->emit(func)); }
        for (u32 i = 0; i < _args.size(); i++) {
            if (arglocs[i].type == LOC_LABEL) {
                arglocs[i] = func.add(new AddressInsn(arglocs[i], ((const ProductType*)argt)->member(i)));
            }
        }
    ...
    ASTFunction::ASTFunction(SourceLocation loc, ref<Env> env, const Type* args_type, const vector<u64>& args,
                             ASTNode* body, i64 name)
        : ASTNode(loc), _env(env), _args_type(args_type), _args(args), _body(body), _name(name), _emitted(false) {
        _body->inc();
    }
    ...
    Location ASTFunction::emit(Function& func) {
        if (_body->is_extern()) {
            ...
            _emitted = true;
            Function& fn = _name == -1 ? func.create_function() : func.create_function(symbol_for(_name));
            _label = fn.label();
            for (u32 i = 0; i < _args.size(); i++) {
                Def* def = _env->find(symbol_for(_args[i]));
                if (def) def->location = fn.add(new LoadArgumentInsn(i, ((ProductType*)_args_type)->member(i)));
            }
    ...
    ASTBlock::ASTBlock(SourceLocation loc, const vector<ASTNode*>& exprs) : ASTNode(loc), _exprs(exprs) {
    ...
    const Type* ASTIf::lazy_type() {
        if (_cond->type() == ERROR || _if_true->type() == ERROR || _if_false->type() == ERROR) return ERROR;
        if (unify(_cond->type(), BOOL) != BOOL) {
            err(_cond->loc(), "Expected condition of type 'bool', given '", _cond->type(), "'.");
            return ERROR;
        }
        const Type *left = _if_true->type(), *right = _if_false->type();
        const Type* t = unify(left, right);
        if (!t) {
            err(loc(), "Could not unify types for branches of if expression: '", left, "' and '", right, "'.");
            return ERROR;
        }
    ...
    ASTWhile::ASTWhile(SourceLocation loc, ASTNode* cond, ASTNode* body) : ASTNode(loc), _cond(cond), _body(body) {
        _cond->inc();
        _body->inc();
    }
    ...
    const Type* ASTWhile::lazy_type() {
        ...
        if (!t) {
            err(loc(), "Invalid condition in 'while' statement: '", _cond->type(), "'.");
            return ERROR;
        }
    ...
    ASTIsEmpty::ASTIsEmpty(SourceLocation loc, ASTNode* list) : ASTUnary(loc, list) {}

    const Type* ASTIsEmpty::lazy_type() {
        if (_child->type() == ERROR) return ERROR;
        const Type* ct = _child->type();
        if (ct->kind() != KIND_LIST && !ct->concrete()) ct = unify(ct, find<ListType>(find<TypeVariable>()));
        if (!ct || (ct->kind() != KIND_LIST && ct != VOID)) {
            err(_child->loc(), "Invalid argument to 'empty?' expression: '", _child->type(), "'.");
            return ERROR;
        }
        return BOOL;
    }
    ...
    ASTHead::ASTHead(SourceLocation loc, ASTNode* list) : ASTUnary(loc, list) {}
    ...
    const Type* ASTHead::lazy_type() {
        ...
        if (!ct || ct->kind() != KIND_LIST) {
            err(_child->loc(), "Invalid argument to 'head' expression: '", _child->type(), "'.");
            return ERROR;
        }
    ...
    ASTTail::ASTTail(SourceLocation loc, ASTNode* list) : ASTUnary(loc, list) {}
    ...
    const Type* ASTTail::lazy_type() {
        ...
        if ((ct->kind() != KIND_LIST && !ct->concrete()) || ct == VOID)
            ct = unify(ct, find<ListType>(find<TypeVariable>()));
        if (!ct || ct->kind() != KIND_LIST) {
            err(_child->loc(), "Invalid argument to 'tail' expression: '", _child->type(), "'.");
            return ERROR;
        }
        return _child->type();
    }
    ...
    ASTCons::ASTCons(SourceLocation loc, ASTNode* first, ASTNode* rest) : ASTBinary(loc, first, rest) {}
    ...
    const Type* ASTCons::lazy_type() {
        ...
        if (rest == VOID) return find<ListType>(first);
            ...
            if (rest->kind() == KIND_TYPEVAR) return unify(rest, find<ListType>(first));
            ...
            if (unify(first, element) != element) {
                err(_left->loc(), "Invalid arguments to 'cons' expression: '", first, "' and '", rest, "'.");
                return ERROR;
            }
            return rest;
        }
    ...
    const Type* ASTLength::lazy_type() {
        ...
        if (unify(_child->type(), STRING) != STRING &&
            unify(_child->type(), find<ListType>(find<TypeVariable>()))->kind() != KIND_LIST) {
            err(_child->loc(), "Argument to 'length' expression must be string or list, ", "given '", _child->type());
            return ERROR;
        }
        return INT;
    }

    ASTLength::ASTLength(SourceLocation loc, ASTNode* child) : basil::ASTUnary(loc, child) {}
    ...
    Location ASTLength::emit(Function& func) {
        ...
        if (_child->type() == STRING) label.label_index = find_label("_strlen");
            ...
            label.label_index = find_label("_listlen");
    ...
    ASTDisplay::ASTDisplay(SourceLocation loc, ASTNode* node) : ASTUnary(loc, node) {}

    Location ASTDisplay::emit(Function& func) {
        const char* name;
        if (_child->type() == INT) name = "_display_int";
        else if (_child->type() == SYMBOL)
            name = "_display_symbol";
        else if (_child->type() == BOOL)
            name = "_display_bool";
        else if (_child->type() == STRING)
            name = "_display_string";
        else if (_child->type() == find<ListType>(INT))
            name = "_display_int_list";
        else if (_child->type() == find<ListType>(SYMBOL))
            name = "_display_symbol_list";
        else if (_child->type() == find<ListType>(BOOL))
            name = "_display_bool_list";
        else if (_child->type() == find<ListType>(STRING))
            name = "_display_string_list";
        else if (_child->type() == VOID)
            name = "_display_int_list";
        vector<Location> args;
        args.push(_child->emit(func));
        Location label;
    ...
    ASTNativeCall::ASTNativeCall(SourceLocation loc, const string& func_name, const Type* ret,
                                 const vector<ASTNode*>& args, const vector<const Type*>& arg_types)
    ...
    Location ASTNativeCall::emit(Function& func) {
        vector<Location> args;
        for (int i = 0; i < _args.size(); i++) args.push(_args[i]->emit(func));
        Location label;
        label.type = LOC_LABEL;
    ...
    ASTAssign::ASTAssign(SourceLocation loc, const ref<Env> env, u64 dest, ASTNode* src)
        : ASTUnary(loc, src), _env(env), _dest(dest) {}
    ...
    const Type* ASTAssign::lazy_type() {
        ...
        if (src_type == ERROR || dest_type == ERROR) return ERROR;
    ...
    const Type* ASTAnnotate::lazy_type() {
        ...
        if (!inferred) {
            err(_value->loc(), "Could not assign type '", _type, "' to value of incompatible type '", _value->type(),
                "'.");
            return ERROR;
        }
        return inferred;
