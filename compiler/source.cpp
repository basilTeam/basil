...
namespace basil {
    ...
    void Source::calc_lines() {
        ...
        for (string* s : _sections) { add_lines(s); }
    }
    ...
    Source::Source(const Source& other) {
        for (string* s : _sections) delete s;
        _sections.clear();
        for (string* s : other._sections) { _sections.push(new string(*s)); }
        calc_lines();
    }
    ...
    Source::View::View(const Source* const src, u32 line, u32 column) : _src(src), _line(line), _column(column) {}
    ...
    u8 Source::View::read() {
        ...
        _column++;
        if (_column >= _src->line(_line).size()) _column = 0, _line++;
