...
namespace basil {
    ...
    static const char* TOKEN_NAMES[NUM_TOKEN_TYPES] = {"none",           "int",          "symbol",        "string",
                                                       "coeff",          "float",        "left paren",    "right paren",
                                                       "access bracket", "left bracket", "right bracket", "left brace",
                                                       "right brace",    "semicolon",    "dot",           "comma",
                                                       "colon",          "pipe",         "plus",          "minus",
                                                       "quote",          "newline"};
    ...
    Token scan(Source::View& view, bool follows_space) {
        ...
        if (!ch) {
            ...
            if (view.peek() != '"') err({line, u16(view.col())}, "Expected closing quote in string literal.");
            else {
                view.read();
                return Token(T_STRING, {u32(view.pos() - start), start}, line, start_col);
            }
        } else if (ch == '.') {
            ...
            if (isspace(view.peek())) return Token(T_COLON, {1, start}, line, start_col);
                ...
                return Token(T_QUOTE, {1, start}, line, start_col);
            ...
            view.read();
            if (ch == '+' && !isspace(view.peek()) && !issymbolic(view.peek()))
                return Token(T_PLUS, {1, start}, line, start_col);
            else if (ch == '-' && !isspace(view.peek()) && !issymbolic(view.peek()))
            ...
            if (issymbolstart(view.peek()) || view.peek() == '(' || view.peek() == '[' || view.peek() == '{')
            ...
            err({line, u16(view.col())}, "Unexpected character in input '", view.read(), "'.");
        return NONE;
    }

    TokenView::TokenView(vector<Token>& tokens, Source& source, bool repl)
    ...
    void TokenView::expand() {
        ...
        while (view.peek()) _tokens->push(scan(view));
    }
} // namespace basil
