...
namespace basil {
    ...
    NumericType::NumericType(u32 size, bool floating)
        : Type(2659161638339667757ul ^ ::hash(size) ^ ::hash(floating)), _size(size), _floating(floating) {}
    ...
    bool NumericType::operator==(const Type& other) const {
        return other.kind() == KIND_NUMERIC && ((const NumericType&)other)._floating == _floating &&
    ...
    bool NumericType::coerces_to(const Type* other) const {
        ...
        else if (!floating() && other_floating)
            return other_size > size(); // works for power-of-two type sizes
        return false;
    }
    ...
    NamedType::NamedType(const string& name, const Type* base)
        : Type(1921110990418496011ul ^ ::hash(name) ^ base->hash()), _name(name), _base(base) {}
    ...
    bool NamedType::operator==(const Type& other) const {
        return other.kind() == KIND_NAMED && ((const NamedType&)other)._base == _base &&
    ...
    ListType::ListType(const Type* element) : Type(element->hash() ^ 11340086872871314823ul), _element(element) {}
    ...
    bool ListType::operator==(const Type& other) const {
        return other.kind() == kind() && ((const ListType&)other).element() == element();
    }
    ...
    ArrayType::ArrayType(const Type* element)
        : Type(element->hash() ^ 11745103813974897731ul), _element(element), _fixed(false), _size(0) {}

    ArrayType::ArrayType(const Type* element, u32 size)
        : Type(element->hash() ^ 5095961086520768179ul * ::hash(size)), _element(element), _fixed(true), _size(size) {}
    ...
    const Type* ArrayType::concretify() const {
        return _fixed ? find<ArrayType>(_element->concretify(), _size) : find<ArrayType>(_element->concretify());
    }
    ...
    bool ArrayType::operator==(const Type& other) const {
        return other.kind() == kind() && ((const ArrayType&)other).element() == element() &&
    ...
    bool ArrayType::coerces_to(const Type* other) const {
        ...
        if (other->kind() == KIND_ARRAY) {
            if (((const ArrayType*)other)->element()->concrete() && !element()->concrete()) return false;
            if (((const ArrayType*)other)->fixed() && !fixed()) return false;
            return ((const ArrayType*)other)->element() == element() ||
                   !((const ArrayType*)other)->element()->concrete() && concrete();
        }

        if (fixed() && other->kind() == KIND_PRODUCT && ((const ProductType*)other)->count() == count()) {
    ...
    IntersectType::IntersectType(const set<const Type*>& members)
        ...
        for (const Type* t : _members)
            if (t->kind() == KIND_FUNCTION) _has_function = true;
    }
    ...
    ProductType::ProductType(const vector<const Type*>& members) : Type(vector_hash(members)), _members(members) {}
    ...
    bool DictType::coerces_to(const Type* other) const {
        ...
        bool k = !((const DictType*)other)->_key->concrete(), v = !((const DictType*)other)->_value->concrete();

        return k && v || k && _value->coerces_to(((const DictType*)other)->_value) ||
    ...
    bool DictType::operator==(const Type& other) const {
        return other.kind() == KIND_DICT && ((const DictType&)other)._key == _key &&
    ...
    FunctionType::FunctionType(const Type* arg, const Type* ret)
        : Type(arg->hash() ^ ret->hash() ^ 17623206604232272301ul), _arg(arg), _ret(ret) {}
    ...
    u32 FunctionType::arity() const {
        return _arg->kind() == KIND_PRODUCT ? ((const ProductType*)_arg)->count() : 1;
    }
    ...
    AliasType::AliasType() : Type(9323462044786133851ul) {}
    ...
    MacroType::MacroType(u32 arity) : Type(18254210403858406693ul ^ ::hash(arity)), _arity(arity) {}
    ...
    bool MacroType::operator==(const Type& other) const {
        return other.kind() == kind() && ((const MacroType&)other).arity() == arity();
    }
    ...
    RuntimeType::RuntimeType(const Type* base) : Type(base->hash() ^ 5857490642180150551ul), _base(base) {}
    ...
    bool RuntimeType::operator==(const Type& other) const {
        return other.kind() == kind() && *((const RuntimeType&)other).base() == *base();
    }
    ...
    TypeVariable::TypeVariable(u32 id) : Type(::hash(id) ^ 3860592187614349697ul), _id(id) {}

    TypeVariable::TypeVariable() : TypeVariable(create_typevar()) {}
    ...
    void TypeVariable::format(stream& io) const {
        ...
        if (type_variables[_id]->concrete()) write(io, "(", type_variables[_id], ")");
    }
    ...
    const Type *INT = find<NumericType>(64, false), *FLOAT = find<NumericType>(64, true),
               *SYMBOL = find<SingletonType>("symbol"), *VOID = find<SingletonType>("void"),
               *ERROR = find<SingletonType>("error"), *TYPE = find<SingletonType>("type"), *ALIAS = find<AliasType>(),
               *BOOL = find<SingletonType>("bool"), *ANY = find<SingletonType>("any"),
               *STRING = find<SingletonType>("string"), *MODULE = find<SingletonType>("module");
    ...
    const Type* unify(const Type* a, const Type* b, bool coercing, bool converting) {
        ...
        if (a->kind() == KIND_TYPEVAR) {
            if (!((const TypeVariable*)a)->actual()->concrete()) {
                if (b->concrete() || b->kind() != KIND_TYPEVAR) return ((const TypeVariable*)a)->bind(b), b;
                else if (b > a)
        ...
        if (b->kind() == KIND_TYPEVAR) {
            if (!((const TypeVariable*)b)->actual()->concrete()) {
                if (a->concrete() || a->kind() != KIND_TYPEVAR) return ((const TypeVariable*)b)->bind(a), a;
                else if (a > b)
        ...
        if (a->kind() == KIND_LIST && b->kind() == KIND_LIST) {
            const Type* elt = unify(((const ListType*)a)->element(), ((const ListType*)b)->element());
        ...
        if (a->kind() == KIND_PRODUCT && b->kind() == KIND_PRODUCT) {
            ...
            if (((const ProductType*)a)->count() != ((const ProductType*)b)->count()) return nullptr;
            for (u32 i = 0; i < ((const ProductType*)a)->count(); i++) {
                const Type* member = unify(((const ProductType*)a)->member(i), ((const ProductType*)b)->member(i));
        ...
        if (a->kind() == KIND_FUNCTION && b->kind() == KIND_FUNCTION) {
            const Type* arg = unify(((const FunctionType*)a)->arg(), ((const FunctionType*)b)->arg());
            const Type* ret = unify(((const FunctionType*)a)->ret(), ((const FunctionType*)b)->ret());
