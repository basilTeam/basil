...
namespace basil {
	...
	Insn::Insn(InsnType kind):
		_kind(kind), _loc(loc_none()), _func(nullptr) {}
	...
	void Function::liveness() {
		...
		while (changed) {
			...
			for (i64 i = _insns.size() - 1; i >= 0; i --) {
				...
				in->liveout();
				if (in->in().size() != initial_in || in->out().size() != initial_out)
					changed = true;
			}
		}
	...
	void Function::to_registers() {
		...
		for (i64 i = 0; i < _insns.size(); i ++) {
			for (u32 l : _insns[i]->out()) {
				if (_insns[i]->in().find(l) == _insns[i]->in().end())
					ranges.put(l, { i, -1 });
			}
			for (u32 l : _insns[i]->in()) {
				if (_insns[i]->out().find(l) == _insns[i]->out().end())
					ranges[l].second = i;
			}
		}
	...
	Function::Function(u32 label):
		_stack(0), _label(label), _end(next_label()) {
	...
	StoreInsn::StoreInsn(Location dest, Location src):
		Insn(STORE_INSN), _dest(dest), _src(src) {}
	...
	AddressInsn::AddressInsn(Location src, const Type* t):
		Insn(ADDRESS_INSN), _src(src), _type(t) {}
	...
	BinaryInsn::BinaryInsn(InsnType kind, const char* name, Location left, 
		Location right):
	...
	BinaryMathInsn::BinaryMathInsn(InsnType kind, const char* name, Location left,
		Location right):
		BinaryInsn(kind, name, left, right) {}

	AddInsn::AddInsn(Location left, Location right):
		BinaryMathInsn(ADD_INSN, "+", left, right) {}
	...
	SubInsn::SubInsn(Location left, Location right):
		BinaryMathInsn(SUB_INSN, "-", left, right) {}
	...
	MulInsn::MulInsn(Location left, Location right):
		BinaryMathInsn(MUL_INSN, "*", left, right) {}
	...
	DivInsn::DivInsn(Location left, Location right):
		BinaryMathInsn(DIV_INSN, "/", left, right) {}
	...
	RemInsn::RemInsn(Location left, Location right):
		BinaryMathInsn(REM_INSN, "%", left, right) {}

	void RemInsn::emit() {
		rem(_loc, _left, _right);
	}

	BinaryLogicInsn::BinaryLogicInsn(InsnType kind, const char* name, Location left,
		Location right):
		BinaryInsn(kind, name, left, right) {}
	...
	AndInsn::AndInsn(Location left, Location right):
		BinaryLogicInsn(AND_INSN, "and", left, right) {}
	...
	OrInsn::OrInsn(Location left, Location right):
		BinaryLogicInsn(OR_INSN, "or", left, right) {}
	...
	XorInsn::XorInsn(Location left, Location right):
		BinaryLogicInsn(XOR_INSN, "xor", left, right) {}
	...
	BinaryEqualityInsn::BinaryEqualityInsn(InsnType kind, const char* name, 
	...
	EqualInsn::EqualInsn(Location left, Location right):
		BinaryEqualityInsn(EQ_INSN, "==", left, right) {}
	...
	InequalInsn::InequalInsn(Location left, Location right):
		BinaryEqualityInsn(NOT_EQ_INSN, "!=", left, right) {}

	void InequalInsn::emit() {
		not_equal(_loc, _left, _right);
	}

	BinaryRelationInsn::BinaryRelationInsn(InsnType kind, const char* name, 
		Location left, Location right):
		BinaryInsn(kind, name, left, right) {}
	...
	LessInsn::LessInsn(Location left, Location right):
		BinaryRelationInsn(LESS_INSN, "<", left, right) {}
	...
	LessEqualInsn::LessEqualInsn(Location left, Location right):
		BinaryRelationInsn(LESS_EQ_INSN, "<=", left, right) {}
	...
	GreaterInsn::GreaterInsn(Location left, Location right):
		BinaryRelationInsn(GREATER_INSN, ">", left, right) {}
	...
	void RetInsn::liveout() {
		...
		if (_src.type == LOC_LOCAL) 
			in().insert(_src.local_index);
	}

	LoadArgumentInsn::LoadArgumentInsn(u32 index, const Type* type):
		Insn(LOAD_ARG_INSN), _index(index), _type(type) {}
	...
	void CallInsn::liveout() {
		for (const Location& loc : _args) if (loc.type == LOC_LOCAL)
			in().insert(loc.local_index);
		if (_fn.type == LOC_LOCAL) in().insert(_fn.local_index);
		if (_loc.type == LOC_LOCAL) in().erase(_loc.local_index);
	}

	Label::Label(u32 label):
		Insn(LABEL), _label(label) {}
	...
	IfZeroInsn::IfZeroInsn(u32 label, Location cond):
		Insn(IFZERO_INSN), _label(label), _cond(cond) {}
