...
namespace basil {
    Def::Def(bool is_macro_in, bool is_procedure_in, bool is_infix_in, u8 arity_in, u8 precedence_in)
        : Def(Value(), is_macro_in, is_procedure_in, is_infix_in, arity_in, precedence_in) {}

    Def::Def(Value value_in, bool is_macro_in, bool is_procedure_in, bool is_infix_in, u8 arity_in, u8 precedence_in)
        : value(value_in), is_macro(is_macro_in), is_proc(is_procedure_in), is_infix(is_infix_in), arity(arity_in),
    ...
    Env::Env(const ref<Env>& parent) : _parent(parent), _runtime(false) {}
    ...
    void Env::infix_macro(const string& name, const Value& value, u8 arity, u8 precedence) {
        _defs[name] = Def(value, true, true, true, arity, precedence);
    }

    const Def* Env::find(const string& name) const {
        auto it = _defs.find(name);
        if (it == _defs.end()) return _parent ? _parent->find(name) : nullptr;
        else
            return &it->second;
    }
    ...
    void Env::format(stream& io) const {
        write(io, "{");
        bool first = true;
        for (auto& def : _defs) write(io, first ? "" : " ", def.first), first = false;
        write(io, "}");
